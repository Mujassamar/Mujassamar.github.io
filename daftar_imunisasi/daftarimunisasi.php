...
<head>
  ...
  <title>DATA PENDAFTARAN IMUNISASI</title>
  <style type="text/css">
      body{
...
<div id="content1" style="margin-left: 230px; margin-top: 130px;">
  <h1>Data Pendaftaran Imunisasi</h1>
  ...
  <table id="ttable" border="0" class="table table-hover table-light table-striped">
    <thead class="" style="background-color:#394360;">
        <tr class="text-white text-center align-middle">
            <th class="align-middle" style="width: 10%;">ID Pendaftaran</th>
            <th class="align-middle">NIK Anak</th>
            <th class="align-middle">Nama Anak</th>
            <th class="align-middle">Tanggal Lahir Anak</th>
  ...
  <script type="text/javascript">
    ...
    //   $("#id_imun").load("http://localhost/Aplikasi_EPosyandu/api/Imunisasi/read.php", "func_imun=ambil_data_imun");
      ...
      $.ajax({
		      type : "GET",	
          url : "../api/daftar/read_daftar.php",
          ...
          var content = "";
            for (let index = 0; index < data.length; index++) {
              ...
              content+="<tr>";
              content+= "<td class='text-center'>"+element.id_pendaftaran+"</td>"+
              ...
              '<td><button onclick="window.location.href=\'edit_daftar.php?id_pendaftaran='+ element.id_pendaftaran +'\'" class="btn btn-info" style="padding: 0px 10px 0px 10px;">EDIT</button></td>' +
              '<td><button class="tdelete btn btn-danger" style="padding: 0px 10px 0px 10px;" value="'+element.id_pendaftaran+'" >HAPUS</button></td>'
              content+="</tr>";
            }
      ...
      $(document).on('click', '.tdelete', function(){
        ...
        if(yakin == true){
          $.ajax({
          type : "POST",
					url : "../api/daftar/delete_daftar.php",
					data : {func_daftar : "delete", id_pendaftaran : $(this).val()},
					cache: false,
					success: function(msg){
